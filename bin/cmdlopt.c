...
//
// This returns a string that is the full path to a lib/ directory by
// assuming that the program running is in a relative path like ../bin
//
static inline char *GetLibDir(void) {
    ...
    //      LibDir = "/usr/game/lib"
    //
    char *libDir = realpath(LibDir, 0);

    // Now libDir may be found or not.  All we can do is try.
...
//
static void SpewHelp(int fd, const char *opt) {
    ...
    const char *run = "/" PACKAGE_NAME "/misc/" PROGRAM_NAME "Help";

    char *libDir = GetLibDir();

    size_t len = strlen(libDir) + strlen(run) + 1;
    char *path = calloc(1, len);
    ASSERT(path, "calloc(1,%zu) failed", len);
    snprintf(path, len, "%s%s", libDir, run);

    if(access(path, R_OK | W_OK) != 0) {
        // TODO: We considered keeping on running after this failure, but
        // this is simpler and, we think, expected behavior.
        fprintf(stderr, "program " PROGRAM_NAME "Help was "
                "not found in \"%s\"\n",
                path);
...
void help(void) {
    SpewHelp(STDOUT_FILENO, "-H");
}

void usage(void) {
    SpewHelp(STDOUT_FILENO, "-u");
}
